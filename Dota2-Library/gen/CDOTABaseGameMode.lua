...
function CDOTABaseGameMode:AddRealTimeCombatAnalyzerQuery(queryTable, player, queryName) return 0 end
...
function CDOTABaseGameMode:AllocateFowBlockerRegion(minX, minY, maxX, maxY, gridSize) return {} end

--- Get if weather effects are disabled on the client.  
--- Env: only in `server`
---@return bool
function CDOTABaseGameMode:AreWeatherEffectsDisabled() return true end
...
--- Env: only in `server`
---@return bool
function CDOTABaseGameMode:GetAlwaysShowPlayerInventory() return true end

--- Get whether player names are always shown, regardless of client setting.  
--- Env: only in `server`
---@return bool
function CDOTABaseGameMode:GetAlwaysShowPlayerNames() return true end

--- Are in-game announcers disabled?  
--- Env: only in `server`
---@return bool
function CDOTABaseGameMode:GetAnnouncerDisabled() return true end

--- Is the announcer announcing the mode / saying Choose Your Hero on start of custom games disabled?  
--- Env: only in `server`
---@return bool
function CDOTABaseGameMode:GetAnnouncerGameModeAnnounceDisabled() return true end

--- Set a different camera distance; dota default is 1134.  
--- Env: only in `server`
---@return float
function CDOTABaseGameMode:GetCameraDistanceOverride() return 0 end

--- Get current derived stat value constant.  
--- Env: only in `server`
---@param derivedStatType AttributeDerivedStats
---@return float
function CDOTABaseGameMode:GetCustomAttributeDerivedStatValue(derivedStatType) return 0 end

--- Get the current rate cooldown ticks down for items in the backpack.  
--- Env: only in `server`
---@return float
function CDOTABaseGameMode:GetCustomBackpackCooldownPercent() return 0 end

--- Get the current custom backpack swap cooldown.  
--- Env: only in `server`
---@return float
function CDOTABaseGameMode:GetCustomBackpackSwapCooldown() return 0 end

--- Turns on capability to define custom buyback cooldowns.  
--- Env: only in `server`
---@return bool
function CDOTABaseGameMode:GetCustomBuybackCooldownEnabled() return true end

--- Turns on capability to define custom buyback costs.  
--- Env: only in `server`
---@return bool
function CDOTABaseGameMode:GetCustomBuybackCostEnabled() return true end

--- Get the topbar score display value for dire.  
--- Env: only in `server`
---@return int
function CDOTABaseGameMode:GetCustomDireScore() return 0 end

--- Get the current custom glyph cooldown.  
--- Env: only in `server`
---@return float
function CDOTABaseGameMode:GetCustomGlyphCooldown() return 0 end

--- Allows definition of the max level heroes can achieve (default is 25).  
--- Env: only in `server`
---@return int
function CDOTABaseGameMode:GetCustomHeroMaxLevel() return 0 end

--- Get the topbar score display value for radiant.  
--- Env: only in `server`
---@return int
function CDOTABaseGameMode:GetCustomRadiantScore() return 0 end

--- Get the current custom scan cooldown.  
--- Env: only in `server`
---@return float
function CDOTABaseGameMode:GetCustomScanCooldown() return 0 end

--- Get the rate at which the day/night cycle advances (1.0 = default).  
--- Env: only in `server`
---@return float
function CDOTABaseGameMode:GetDaynightCycleAdvanceRate() return 0 end

--- Get the Game Seed passed from the GC.  
--- Env: only in `server`
---@return int
function CDOTABaseGameMode:GetEventGameSeed() return 0 end

--- Get the Event Window Start Time passed from the GC.  
--- Env: only in `server`
---@return uint
function CDOTABaseGameMode:GetEventWindowStartTime() return 0 end

--- Gets the fixed respawn time.  
--- Env: only in `server`
---@return float
function CDOTABaseGameMode:GetFixedRespawnTime() return 0 end

--- Turn the fog of war on or off.  
--- Env: only in `server`
---@return bool
function CDOTABaseGameMode:GetFogOfWarDisabled() return true end

--- Turn the sound when gold is acquired off/on.  
--- Env: only in `server`
---@return bool
function CDOTABaseGameMode:GetGoldSoundDisabled() return true end

--- Returns the HUD element visibility.  
--- Env: only in `server`
---@param element int
---@return bool
function CDOTABaseGameMode:GetHUDVisible(element) return true end

--- Get the maximum attack speed for units.  
--- Env: only in `server`
---@return int
function CDOTABaseGameMode:GetMaximumAttackSpeed() return 0 end

--- Get the minimum attack speed for units.  
--- Env: only in `server`
---@return int
function CDOTABaseGameMode:GetMinimumAttackSpeed() return 0 end

--- Turn the panel for showing recommended items at the shop off/on.  
--- Env: only in `server`
---@return bool
function CDOTABaseGameMode:GetRecommendedItemsDisabled() return true end

--- Returns the scale applied to non-fixed respawn times.  
--- Env: only in `server`
---@return float
function CDOTABaseGameMode:GetRespawnTimeScale() return 0 end
...
--- Env: only in `server`
---@return bool
function CDOTABaseGameMode:GetStashPurchasingDisabled() return true end

--- Hide the sticky item in the quickbuy.  
--- Env: only in `server`
---@return bool
function CDOTABaseGameMode:GetStickyItemDisabled() return true end

--- Override the values of the team values on the top game bar.  
--- Env: only in `server`
---@return bool
function CDOTABaseGameMode:GetTopBarTeamValuesOverride() return true end

--- Turning on/off the team values on the top game bar.  
--- Env: only in `server`
---@return bool
function CDOTABaseGameMode:GetTopBarTeamValuesVisible() return true end

--- Gets whether tower backdoor protection is enabled or not.  
--- Env: only in `server`
---@return bool
function CDOTABaseGameMode:GetTowerBackdoorProtectionEnabled() return true end

--- Are custom-defined XP values for hero level ups in use?  
--- Env: only in `server`
---@return bool
function CDOTABaseGameMode:GetUseCustomHeroLevels() return true end

--- Gets the time from game start during which water runes spawn.  
--- Env: only in `server`
---@return float
function CDOTABaseGameMode:GetWaterRuneLastSpawnTime() return 0 end

--- Const char* pszAbilityName.  
--- Env: only in `server`
---@param abilityName string
---@return bool
function CDOTABaseGameMode:IsAbilityUpgradeWhitelisted(abilityName) return true end

--- Enables or disables buyback completely.  
--- Env: only in `server`
---@return bool
function CDOTABaseGameMode:IsBuybackEnabled() return true end

--- Is the day/night cycle disabled?  
--- Env: only in `server`
---@return bool
function CDOTABaseGameMode:IsDaynightCycleDisabled() return true end
...
--- Env: only in `server`
---@return bool
function CDOTABaseGameMode:ShouldGiveFreeTPOnDeath() return true end
