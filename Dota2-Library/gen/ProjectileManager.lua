...
---@return ProjectileID
function ProjectileManager:CreateLinearProjectile(options) return 0 end

--- Creates a tracking projectile.  
--- Env: only in `server`
---@param options CreateTrackingProjectileOptions
---@return ProjectileID
function ProjectileManager:CreateTrackingProjectile(options) return 0 end
...
---@param projectile ProjectileID
---@return Vector
function ProjectileManager:GetLinearProjectileLocation(projectile) return Vector() end

--- Returns current radius of projectile.  
--- Env: only in `server`
---@param projectile ProjectileID
---@return float
function ProjectileManager:GetLinearProjectileRadius(projectile) return 0 end

--- Returns a vector representing the current velocity of the projectile.  
--- Env: only in `server`
---@param projectile ProjectileID
---@return Vector
function ProjectileManager:GetLinearProjectileVelocity(projectile) return Vector() end

--- Returns current location of projectile.  
--- Env: only in `server`
---@param projectile ProjectileID
---@return Vector
function ProjectileManager:GetTrackingProjectileLocation(projectile) return Vector() end

--- Is this a valid projectile?  
--- Env: only in `server`
---@param value int
---@return bool
function ProjectileManager:IsValidProjectile(value) return true end
